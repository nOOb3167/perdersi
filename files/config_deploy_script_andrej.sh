...
case $1 in
    --build)
        ...
        $SSH "bash -l -c $Q $CMAKE $CMAKE_PARAMS $Q"                                   || psexit cmake configure
        $SSH "cat E:/prog/perdersi/RemPerdersiBuild/ps_marker_remote | grep ON"                   || psexit marker
        $SSH "bash -l -c $Q $CMAKE --build E:/prog/perdersi/RemPerdersiBuild --target INSTALL $Q" || psexit cmake build
