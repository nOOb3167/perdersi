...
def _getmtime(p: pathname_t) -> float:
    try:
        return os_path__getmtime(p)
    except FileNotFoundError:
        return 0.0

def _get_latest(p: dirname_t) -> float:
    ...
    dirs_max = max([_getmtime(v) for v in dirs])
    files_max = max([_getmtime(v) for v in files])

    any_max = max(dirs_max, files_max)
    return any_max

def _get_latest_str(p: dirname_t) -> str:
    any_max = _get_latest(p)
    any_max_s = datetime__datetime.utcfromtimestamp(any_max).strftime('%Y-%m-%d %H:%M:%S UTC')
    return any_max_s

print(_get_latest_str('..'))
